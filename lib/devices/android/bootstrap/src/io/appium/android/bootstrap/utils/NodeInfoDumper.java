...
public abstract class NodeInfoDumper {

    private static final String LOGTAG = NodeInfoDumper.class.getSimpleName();
     ...
     * @since API Level 16
     */
    public static void dumpWindowHierarchy(String fileName) {
      try {
        ReflectionUtils utils = new ReflectionUtils();
          AccessibilityNodeInfo root = utils.getAccessibilityRootNode();
             //     getAutomatorBridge().getQueryController().getAccessibilityRootNode();
          if(root != null) {
              final UiDevice d = UiDevice.getInstance();
              Point size = new Point(d.getDisplayWidth(), d.getDisplayHeight());
              NodeInfoDumper.dumpWindowToFile(root,
                      new File(new File(Environment.getDataDirectory(), "local/tmp"), fileName), 
                      d.getDisplayRotation(), size.x.intValue(), size.y.intValue());
          }
      } catch (Exception e) {
    ...
    public static void dumpWindowToFile(AccessibilityNodeInfo root, File dumpFile, int rotation,
        ...
        Logger.debug("Fetch time: " + (endTime - startTime) + "ms");
    }
...
//              }
                if (child.isVisibleToUser()) {
                    ...
                    Logger.debug(String.format("Skipping invisible child: %s", child.toString()));
                }
            } else {
                Logger.debug(String.format("Null child %d/%d, parent: %s",
